...
WHERE ano BETWEEN 2018 AND 2026
  AND trimestre = 4
GROUP BY ano, sigla_uf, V2010, V2007, VD3004
ORDER BY ano, sigla_uf, V2010, V2007, VD3004
"
...
df_completo <- df_pnadc %>%
  ...
  ungroup() %>% 
  select(ano, regiao, sigla_uf, sexo, raca_cor, nivel_instrucao, populacao_estimada, populacao_total_estado, percentagem) %>%
  arrange(ano, regiao, sigla_uf, sexo, raca_cor, nivel_instrucao)

# ==============================================================================
# 5. EXPORTAÇÃO PARA EXCEL E VISUALIZAÇÃO
# ==============================================================================
caminho_ficheiro <- "~/pnadc_dados_sociodemograficos_2018_2026.xlsx"
